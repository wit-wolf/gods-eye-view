...
| **Ancora centres (local GeoJSON)** | Optional retail-centre pins (`public/sites/ancora-centres.geojson`) | Operator-supplied / EXAMPLE placeholders until live Ancora rows | Label EXAMPLE features with `example: true` |
...
- **Ancora / zoning / census (operator GeoJSON).** Volo reads optional same-origin files under `public/sites/`:
  - `ancora-centres.geojson` — Ancora Data Layer (amber pins). Ship EXAMPLE centres with `example: true` until live DB rows replace them. Occupancy / GLA / mandate appear only when properties are present — never invented.
