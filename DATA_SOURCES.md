...
| **Municipal zoning / SDF (local GeoJSON)** | Optional pin∩polygon join (`public/sites/zoning.geojson`) | Municipal SDF licence (operator-supplied) | Show source in card when present |
| **Census / ward polygons (local GeoJSON)** | Optional ward identifiers only (`public/sites/census-wards.geojson`) — **no LSM/income invented** | Stats SA / municipal open data (operator-supplied) | Ward name/code only |
...
- **Ancora / zoning / census (operator GeoJSON).** Volo reads optional same-origin files under `public/sites/`:
  ...
  - `zoning.geojson` — municipal SDF polygons; research cards join the open pin and show `zone_name` / `zone_code` (and aliases). Empty or missing → “No zoning layer loaded”. Copy schema from `zoning.geojson.example`.
  - `census-wards.geojson` — Stats SA / municipal ward polygons for **identifiers only** (ward name/code, municipality). Empty or missing → “Stats SA not wired…”. **Never invent LSM, household income, or AfriGIS paid demographics.** Copy schema from `census-wards.geojson.example`.
