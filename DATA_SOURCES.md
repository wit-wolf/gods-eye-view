...
| **TomTom Traffic API** (flow vector tiles) + Search / Routing (Sites) | Live congestion coloring; Sites reverse-geocode, Places nearby, reachable-range (optional, BYOK) | [TomTom for Developers terms](https://developer.tomtom.com) (proprietary, your own key; quotas depend on the current account plan) | "Traffic flow data © TomTom" — registered when live mode activates |
...
| **Ancora centres (local GeoJSON)** | Retail-centre pins. Live dump: `public/sites/ancora-centres.geojson` (**gitignored**). Template: `ancora-centres.example.geojson` | Operator PropertyCentral export + Places geocode (ZA/ZW); EXAMPLE placeholders in repo | Label EXAMPLE with `example: true`; show geocoded (not surveyed) |
| **Municipal zoning / SDF (local GeoJSON)** | Optional pin∩polygon join (`public/sites/zoning.geojson`, **gitignored**). Template: `zoning.example.geojson` | Municipal SDF licence (operator-supplied) | Show source in card when present |
| **George Municipality Integrated Zoning** | Optional bbox overlay when local zoning file is missing — `/api/george-zoning` → CITP FeatureServer/16 envelope around the open pin (`zoning`, `zoning_code`, `land_use`, `town`). **Not** a national layer; does **not** download all ~54k polygons | George Municipality GIS (courtesy; municipal open GIS) | "George Municipality Integrated Zoning (CITP)" — honest fail if CORS/network blocks → drop a GeoJSON extract |
| **Census / ward / SAL (local GeoJSON)** | Optional ward/`SAL_CODE` identifiers only (`public/sites/census-wards.geojson`, **gitignored**). Template: `census-wards.example.geojson`. **Census 2022 SAL is request-only; income/employment 2022 withheld** — no invented LSM/income | Stats SA 2011 SAL/wards + [SuperWEB2](https://superweb.statssa.gov.za/) CSV join (operator-supplied) | Ward / SAL code only; card states Census 2022 small-area not public |
| **City of Cape Town Open Data — zoning** | Documented alternate municipal REST (operator may export GeoJSON for local drop) | City of Cape Town Open Data terms | Attribute City of Cape Town when used |
| **City of Johannesburg zoning extract** | Paid / request extract (not free national feed) — operator drops GeoJSON locally | City of Johannesburg / licensed extract | Attribute Jo'burg source when used |
| **Western Cape Government Spatial Data Warehouse** | Provincial spatial layers (operator download → local GeoJSON) | WCG Spatial Data Warehouse terms | Attribute WCG when used |
...
- **TomTom Traffic.** Optional and BYOK: without `TOMTOM_API_KEY` the traffic layer runs its built-in simulation and no TomTom data (or attribution) appears. With a key, flow vector tiles are fetched through the server-side `/api/tomtom` proxy (120 s cache + a configurable daily tile-budget governor, default 40,000 requests) and the "Traffic flow data © TomTom" credit is registered in the Data attribution popover the moment live mode activates. Sites research briefs also call `/api/tomtom/reachable-range` (5/10/15 min drive-time rings), `/api/tomtom/reverse-geocode` (pin locality), and `/api/tomtom/nearby-poi` (retail competitor ring) — all share a 6 h cache and `TOMTOM_DAILY_ROUTE_BUDGET` (default 800). Keyless / over budget shows honest unavailable or labeled Nominatim / Google Places fallbacks — never invented %. Flow tiles stay on the classic relative path. Set those governors within the current allowance for your TomTom account (e.g. Evaluation); the application defaults are safety limits, not a promise of free quota. TomTom data is served live and cached only transiently (≤120 s tile TTL / ≤6 h route/search TTL under `.gev-cache/`, gitignored) — it is not bundled or redistributed. One 23 KB point-in-time tile snapshot is committed as a decode-test fixture (`src/data/fixtures/`, © TomTom, never served to the app).
...
- **Ancora / zoning / census (operator GeoJSON + George overlay).** Volo reads optional same-origin files under `public/sites/`:
  - `ancora-centres.geojson` — **live PropertyCentral dump (gitignored)**. Werner drops ~17 COMMERCIAL_RETAIL ACTIVE rows locally after Places geocode (lat/lon were NULL in DB). Occupancy is **unit counts** (`units_occupied` / `units`) — never invent tenants. Show `geocoded_name` / `geocoded_address` plus “geocoded (not surveyed)”. Known Places collisions (Whitelands/Wijnland, Build It Tembisa/Marula Mile, Heron Banks, Bram Fischerville) are listed on the card.
  - `ancora-centres.example.geojson` — committed EXAMPLE template (`example: true`) used when the live file is absent.
  - `zoning.geojson` — **gitignored** municipal SDF polygons; research cards join the open pin and show zone name/code (and aliases). Copy schema from `zoning.example.geojson`.
  - If `zoning.geojson` is missing/empty, Volo optionally bbox-queries **George Municipality Integrated Zoning** (CITP FeatureServer/16) via `/api/george-zoning?lat=&lon=` — small envelope + `resultRecordCount` cap; attributes George Municipality. **There is no national zoning GeoJSON.** Honest fail on CORS/network → drop a GeoJSON extract. Cape Town Open Data zoning REST, Joburg paid extracts, and WCG Spatial Data Warehouse are documented sources for operator-side extracts — not auto-downloaded here.
  - `census-wards.geojson` — **gitignored** Stats SA **2011** SAL/ward polygons (+ optional SuperWEB2 CSV attributes) for **identifiers only** (`ward_*`, `SAL_CODE`, municipality). **Census 2022 small-area is not public** (request-only; income/employment 2022 withheld). Empty/missing card copy: “Census 2022 small-area not public; drop 2011 SAL/ward GeoJSON + SuperWEB2 CSV to enable.” **Never invent LSM, household income, or AfriGIS paid demographics.** Template: `census-wards.example.geojson`.
  - Do **not** use wazimap.co.za (404 as of 2026-09-02).
  No paid AfriGIS subscription is required for these joins.
